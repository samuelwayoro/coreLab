...
public class DojoStream {
    ...
    public static void main(String... args) {
        ...
        //créer un stream à partir d'un tableau grâce à la classe utilitaire Arrays
        String[] mots = {"one", "three", "two"};
        Arrays.stream(mots);


        //MAP
        List<Integer> mappedNumbersList = numbers.stream().map(x -> x * x).toList();
        // System.out.println("mappedNumberList "+mappedNumbersList);
        List<String> mappedNamesList = names.stream().map(String::toUpperCase).toList();
        // System.out.println("mappedNamesList "+mappedNamesList);

        //COLLECT
        List<Integer> squaredNumber = numbers.stream().map(x -> x * x).toList();
